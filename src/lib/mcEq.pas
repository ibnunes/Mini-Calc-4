...
type
   ...
   TFnMgr = class (classes.TStringList) // Class Interceptor
      ...
      public
         ...
         function NavigateThroughFunctions(const X, Y : word; var idx : LongInt; fn : TFnGetName; const SWG : boolean; const EXTRAKEYS : TKeySet; graph : TPlotter; par : TFPExpressionParser) : char;
...
function TFnMgr.NavigateThroughFunctions(const X, Y : word; var idx : LongInt; fn : TFnGetName; const SWG : boolean; const EXTRAKEYS : TKeySet; graph : TPlotter; par : TFPExpressionParser) : char;
(* NOTE: in the future, if we need more functions, it will have do work as the VLM. *)

const
   WIDTH  = 38;
   { HEIGHT = 10; }  // Read initial NOTE.
   NAVKEYS = [KEY_FN_SELECT, KEY_FN_DELETE, KEY_FN_EDIT, KEY_FN_EXIT, KEY_FN_UP, KEY_FN_DOWN, KEY_FN_VISIBLE, KEY_FN_GRAPH];
...
begin
   ...
   repeat      
      for i := 0 to pred(self.count) do begin
         ...
         GotoXY(X, Y+i);
         write(AnsiLeftStr(PadRight(IfThen(graph.Equation[i].line.visible, '*', ' ') + 'Y' + IntToStr(i) + '= ' + self[i], WIDTH), WIDTH));
      end;
      ...
      case opt of
         ...
         KEY_FN_DELETE :
            begin
               self[idx] := '';
               graph.SetFunction(idx, '');
               graph.Equation[idx].line.visible := false;
               if graph.WindowVisible then
                  graph.RefreshWindow;
            end;
         ...
         KEY_FN_EDIT :
            begin
               ...
               if equation <> '' then begin
                  self[idx] := equation;
                  graph.SetFunction(idx, equation);
                  if graph.WindowVisible then
                     graph.RefreshWindow;
               end;
            end;
         ...
         KEY_FN_VISIBLE :
            begin
               if self[idx] <> '' then begin
                  graph.Equation[idx].line.visible := not graph.Equation[idx].line.visible;
                  if graph.WindowVisible then
                     graph.RefreshWindow;
               end;
            end;
   ...
   until opt in [KEY_FN_EXIT, KEY_FN_SELECT, KEY_FN_GRAPH] + EXTRAKEYS;
